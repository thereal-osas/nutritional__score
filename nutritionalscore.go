...
func GetNutritionalScore(n NutritionalData, st ScoreType) NutritionalScore {
	...
	if st != Water {
		fruitPoints := n.Fruits.GetPointsst
		fibrePoints := n.Fibre.GetPoints(st)

		negative = n.Energy.GetPoints(st) + n.Sugars + n.SaturatedFattyAcids.GetPoints(st) + n.Sodium.GetPoints(st)
